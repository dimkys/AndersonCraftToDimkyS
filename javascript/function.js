...
function ResetChildrenAndDel(InputElem,OutputElem) {
    for (var k = InputElem.childNodes.length-1; k >=0; k--) {
        try{
            if(InputElem.childNodes[k].id<5) {
                OutputElem.appendChild(InputElem.childNodes[k]);
            }
        }catch (e){
            console.log(e);
        }
    }
    removeChildren(InputElem);
};
function ResetChildrenNoDel(InputElem,OutputElem) {
    for (var k = InputElem.childNodes.length-1; k >=0; k--) {
        try{
                OutputElem.appendChild(InputElem.childNodes[k]);
        }catch (e){
